def predict_stock(self, code):
    ...
    # 특정일의 주식 가격은 data, 특정일 다음 날의 종가 가격은 target에 할당
    ...
    # 현재 날짜의 가격을 통해 다음 날 종가 예측
    predict_price = round(int(rf.predict([today_price])[0]), -2)

    # 예측한 가격이 현재 날짜의 종가보다 높으면 추천하는 글 화면에 출력
    if df.iloc[-1]['close'] < predict_price:
        ...
        if self.get_deposit() > df.iloc[-1]['close']:
            self.send_order("buy", "0001", 1, code, 1, 0, "03")
            self.best[code] = predict_price
            f = open("best.dat", 'wb')
    ...
    for i in self.get_balance():
        if i[1] in self.best.keys():
            self.send_order("sell", "0001", 2, i[0][1:], self.best[i[0][1:]], "00")
